...
class ChestXrayDataset(Dataset):
    def __init__(self, img_path, cfg, transform=None, phase='train', df=None, data_type='detection'):
    ...
    def __getitem__(self, idx):
        if self.phase == 'test':
            ...
            if self.transform:
                for i in range(10):
                    transformed_img, transformed_bboxes, transformed_class_labels = self.transform(image, target['boxes'], labels)
                    if len(transformed_bboxes) > 0:
                        image = transformed_img
                        target['boxes'] = torch.stack(tuple(map(torch.tensor, zip(*transformed_bboxes)))).permute(1, 0)
                        target['boxes'][:,[0,1,2,3]] = target['boxes'][:,[1,0,3,2]]  ## ymin, xmin, ymax, xmax
                        target['labels'] = torch.tensor(transformed_class_labels)
                        break
    ...
    def load_image_and_boxes(self, idx):
        ...
        resize_transform = A.Compose([A.Resize(height=self.cfg.data.img_size, width=self.cfg.data.img_size, p=1.0)],
                                     p=1.0,
        ...
        resized_bboxes = np.stack([bx for bx in resized['bboxes']])
        resized_labels = np.stack(resized['labels'])

        return resized['image'], resized_bboxes, resized_labels, image_id
